...
const classeurByOrder: Record<number, {title:string; href:string}> = {
  ...
  1: { title: "Clients et régions", href: "/resources/sql-data-analyst/clients.csv" },
  3: { title: "Ventes et objectifs", href: "/resources/sql-data-analyst/ventes.csv" },
  4: { title: "Requêtes de départ", href: "/resources/sql-data-analyst/03_requetes_depart.sql" },
...
export default function SqlLearningContent({ moduleId, moduleOrder, completed, passScore, onValidated }: { moduleId:string; moduleOrder:number; completed:boolean; passScore:number; onValidated:()=>void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      contentAPI.lessons("sql-data-analyst", moduleId),
      contentAPI.exercises("sql-data-analyst", moduleId),
      contentAPI.theory("sql-data-analyst"),
      contentAPI.projects("sql-data-analyst"),
    ]).then(([l,e,t,p]) => {
      if (!active) return;
      const rows = l.data || [];
  ...
  const submitProject = async () => {
    ...
    setProjectMessage("");
    try {
      await validationAPI.submitFinalProject(submissionUrl || undefined, submissionNotes || undefined, "sql-data-analyst");
  ...
  return <div className="space-y-8">
    ...
    {classeur && <section className="rounded-3xl border border-cyan-300/20 bg-cyan-500/10 p-5 sm:p-7"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><FlaskConical className="text-cyan-200"/><div><p className="text-xs font-black uppercase tracking-[.16em] text-cyan-100">Ressource SQL pratique</p><h2 className="text-xl font-black text-white">{classeur.title}</h2></div></div><a href={classeur.href} download className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-black text-slate-950"><Download size={16}/> Télécharger la ressource</a></div></section>}
