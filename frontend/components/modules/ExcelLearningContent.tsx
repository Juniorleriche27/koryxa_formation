...
const classeurByOrder: Record<number, {title:string; href:string}> = {
  3: { title: "Laboratoire chunking", href: "/classeurs/llm-rag/03-chunking.ipynb" },
  4: { title: "Laboratoire embeddings", href: "/classeurs/llm-rag/04-embeddings.ipynb" },
  5: { title: "Laboratoire recherche vectorielle", href: "/classeurs/llm-rag/05-vector-search.ipynb" },
  6: { title: "Laboratoire retrieval", href: "/classeurs/llm-rag/06-retrieval.ipynb" },
  8: { title: "Laboratoire évaluation RAG", href: "/classeurs/llm-rag/08-evaluation.ipynb" },
};
...
export default function ExcelLearningContent({ moduleId, moduleOrder, completed, passScore, onValidated }: { moduleId:string; moduleOrder:number; completed:boolean; passScore:number; onValidated:()=>void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      contentAPI.lessons("excel-data-analyst", moduleId),
      contentAPI.exercises("excel-data-analyst", moduleId),
      contentAPI.theory("excel-data-analyst"),
      contentAPI.projects("excel-data-analyst"),
    ]).then(([l,e,t,p]) => {
      if (!active) return;
      const rows = l.data || [];
  ...
  return <div className="space-y-8">
    ...
    {classeur && <section className="rounded-3xl border border-cyan-300/20 bg-cyan-500/10 p-5 sm:p-7"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><FlaskConical className="text-cyan-200"/><div><p className="text-xs font-black uppercase tracking-[.16em] text-cyan-100">Laboratoire pratique</p><h2 className="text-xl font-black text-white">{classeur.title}</h2></div></div><a href={classeur.href} download className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-black text-slate-950"><Download size={16}/> Télécharger le classeur</a></div></section>}
