...
const classeurByOrder: Record<number, {title:string; href:string}> = {
  0: { title: "Sources et référentiels", href: "/resources/power-bi-data-analyst/01_sources_et_referentiels.xlsx" },
  3: { title: "Dictionnaire du modèle", href: "/resources/power-bi-data-analyst/02_modele_donnees.xlsx" },
  5: { title: "Mesures DAX et objectifs", href: "/resources/power-bi-data-analyst/03_dax_kpi.xlsx" },
  9: { title: "Cahier du dashboard", href: "/resources/power-bi-data-analyst/04_cahier_dashboard.xlsx" },
};
...
export default function PowerBiLearningContent({ moduleId, moduleOrder, completed, passScore, onValidated }: { moduleId:string; moduleOrder:number; completed:boolean; passScore:number; onValidated:()=>void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      contentAPI.lessons("power-bi-data-analyst", moduleId),
      contentAPI.exercises("power-bi-data-analyst", moduleId),
      contentAPI.theory("power-bi-data-analyst"),
      contentAPI.projects("power-bi-data-analyst"),
    ]).then(([l,e,t,p]) => {
      if (!active) return;
      const rows = l.data || [];
  ...
  const classeur = classeurByOrder[moduleOrder];

  const submitProject = async () => {
    setSubmittingProject(true);
    setProjectMessage("");
    try {
      await validationAPI.submitFinalProject(submissionUrl || undefined, submissionNotes || undefined, "power-bi-data-analyst");
  ...
  return <div className="space-y-8">
    ...
    {classeur && <section className="rounded-3xl border border-cyan-300/20 bg-cyan-500/10 p-5 sm:p-7"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><FlaskConical className="text-cyan-200"/><div><p className="text-xs font-black uppercase tracking-[.16em] text-cyan-100">Fichier pratique</p><h2 className="text-xl font-black text-white">{classeur.title}</h2></div></div><a href={classeur.href} download className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-black text-slate-950"><Download size={16}/> Télécharger le fichier</a></div></section>}
