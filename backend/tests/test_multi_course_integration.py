...
class MultiCourseIntegrationTests(unittest.TestCase):
    ...
    def test_machine_learning_release_is_complete_and_published(self):
        ...
        self.assertIn("total_points<>40", migration)
        self.assertIn("rubric_total<>60", migration)
        self.assertIn("module_hours<>44", migration)
        self.assertIn("course_hours<>60", migration)
        self.assertIn("SET is_published = TRUE", migration)
    ...
    def test_data_engineering_exercises_resources_are_complete_and_unpublished(self):
        ...
        self.assertNotIn("'project'", migration)
        self.assertEqual(len(list((resources / "starter").glob("*"))), 12)
        self.assertEqual(len(list((resources / "solutions").glob("*"))), 12)
        self.assertTrue((resources / "sales.csv").exists())
        self.assertTrue((resources / "customers.csv").exists())
    ...
    def test_python_data_analyst_errors_and_external_resources_are_restored(self):
        ...
        self.assertIn('const PYODIDE_INDEX_URL = "/pyodide/"', notebook_viewer)
        self.assertIn("Chargement du runtime Python local impossible", notebook_viewer)
        self.assertNotIn("https://unpkg.com/pyodide@0.26.4/", notebook_viewer)
