-- KORYXA FORMATION — STATISTIQUES & DATA SCIENCE AVEC PYTHON — CHANTIER 6
...
BEGIN
    SELECT COUNT(*), COALESCE(SUM(m.platform_points), 0)
    ...
    IF module_count <> 12
       ...
       OR published_course IS NOT TRUE THEN
        RAISE EXCEPTION 'Publication Statistiques incomplète: modules %, leçons %, quiz %, quiz finaux %, projets %, jalons %, points plateforme %, barème projet %, heures %, publié %',
            ...
            project_count, milestone_count, total_points, rubric_total, course_hours, published_course;
    END IF;
END $$;
