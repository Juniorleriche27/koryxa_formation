-- KORYXA FORMATION — DATA ENGINEERING AVEC PYTHON ET SQL — CHANTIER 7
...
WITH c AS (SELECT id FROM public.courses WHERE slug = 'data-engineering-python-sql')
...
SET is_published = TRUE,
    estimated_hours = 64,
    final_project_title = 'Plateforme analytique de ventes avec Python, SQL, dbt et Airflow',
    final_project_description = 'Projet autonome estimé à 16 heures : ingestion de fichiers et API, stockage PostgreSQL, chargements incrémentaux, transformations dbt, orchestration Airflow, contrôles de qualité, observabilité, Docker et alimentation d’un modèle analytique de ventes.'
WHERE slug = 'data-engineering-python-sql';
...
BEGIN
    ...
    IF module_count<>12
       ...
       OR published_course IS NOT TRUE THEN
        RAISE EXCEPTION 'Publication Data Engineering incomplète: modules %, leçons %, exercices %, ressources %, quiz %, quiz finaux %, projets %, jalons %, points %, barème %, heures modules %, heures cours %, publié %',
