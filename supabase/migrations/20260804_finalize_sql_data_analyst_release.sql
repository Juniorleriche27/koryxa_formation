-- KORYXA FORMATION — SQL DATA ANALYST AVEC POSTGRESQL — CHANTIER 6
-- Publication finale, score plateforme sur 40 points et recette complète.
...
SET is_published = TRUE,
    estimated_hours = 26,
    final_project_title = 'Analyse commerciale d’une base PostgreSQL',
    final_project_description = 'Conception d’une base relationnelle, import des données, requêtes analytiques, vues, fonctions de fenêtre, optimisation, sécurité et connexion Power BI.'
WHERE slug = 'sql-data-analyst';
...
BEGIN
    ...
    WHERE c.slug = 'sql-data-analyst' AND m.is_published = TRUE;

    SELECT COUNT(*) INTO lesson_count
    FROM public.lessons l
    JOIN public.modules m ON m.id = l.module_id
    JOIN public.courses c ON c.id = m.course_id
    WHERE c.slug = 'sql-data-analyst' AND l.is_published = TRUE;

    SELECT COUNT(*) INTO exercise_count
    FROM public.exercises e
    JOIN public.courses c ON c.id = e.course_id
    WHERE c.slug = 'sql-data-analyst' AND e.is_published = TRUE;

    SELECT COUNT(*) INTO resource_count
    FROM public.theory_resources r
    JOIN public.courses c ON c.id = r.course_id
    WHERE c.slug = 'sql-data-analyst' AND r.is_published = TRUE;
    ...
    JOIN public.courses c ON c.id = m.course_id
    WHERE c.slug = 'sql-data-analyst' AND q.is_active = TRUE;

    SELECT COUNT(*) INTO project_count
    FROM public.course_projects p
    JOIN public.courses c ON c.id = p.course_id
    WHERE c.slug = 'sql-data-analyst' AND p.is_published = TRUE;
    ...
    JOIN public.courses c ON c.id = p.course_id
    WHERE c.slug = 'sql-data-analyst' AND pm.is_published = TRUE;
    ...
    IF module_count <> 12
       ...
       OR published_course IS NOT TRUE THEN
        RAISE EXCEPTION 'Publication SQL incomplète: modules %, leçons %, exercices %, ressources %, quiz %, quiz finaux %, projets %, jalons %, points plateforme %, barème projet %, publié %',
